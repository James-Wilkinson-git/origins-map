...
export function HallPrintToolbar({
  ...
  useEffect(() => {
    ...
    const printMap = async () => {
      ...
      try {
        ...
        w.document.open();
        w.document.write(
          `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Origins ${safeName}</title>
    ...
    const downloadPng = async () => {
      ...
      try {
        ...
        a.href = dataUrl;
        a.download = `${safeName}_origins.png`;
        a.click();
      } catch (e) {
        console.error(e);
    ...
    const C = L.Control.extend({
      ...
      onAdd() {
        ...
        root.className = "leaflet-bar leaflet-control origins-print-group";

        const printBtn = L.DomUtil.create("a", "", root);
        printBtn.href = "#";
        printBtn.title = "Print hall — fits entire floor on the page";
        printBtn.innerHTML = '<span class="origins-print-glyph">⎙</span>';
        ...
        dlBtn.innerHTML =
          '<span class="origins-print-glyph origins-print-glyph--download" aria-hidden="true">' +
  ...
  }, [map, bounds, filenameBase, viewBoxSize?.vw, viewBoxSize?.vh]);
